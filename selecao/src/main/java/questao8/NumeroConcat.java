package questao8;

/**
 * Dados dois numeros inteiros A e B, crie um terceiro inteiro C seguindo as seguintes
	regras:
	- O primeiro número de C é o primeiro número de A;
	- O segundo número de C é o primeiro número de B;
	- O terceiro número de C é o segundo número de A;
	- O quarto número de C é o segundo número de B;
	- Caso os números de A ou B sejam de tamanhos diferentes, completar C com o restante
		dos números do inteiro maior. Ex: A = 10256, B = 512, C deve ser 15012256.
	- Caso C seja maior que 1.000.000, retornar -1
 * @author Alexandre Mutti
 *
 */

public class NumeroConcat {
	
	private static final Integer limit = 1000000;
	
	private static int run(char a[], char b[]) {
		
		StringBuilder sb = new StringBuilder();
		int maxLength = Math.max(a.length, b.length);
		
		for (int i = 0; i < maxLength; i++) {
			
			if(a.length > i) {
				sb.append(a[i]);
			} 
			
			if (b.length > i) {
				sb.append(b[i]);
			}
		}
		
		Integer result = Integer.valueOf(sb.toString());
		
		if(result > limit) {
			return -1;
		} else {
			return result;
		}		
	}
	
	public static int numBuilder(Integer a, Integer b) {
		
		char[] charArrayA = a.toString().toCharArray();
		char[] charArrayB = b.toString().toCharArray();
		
		return run(charArrayA, charArrayB);
	}
}
